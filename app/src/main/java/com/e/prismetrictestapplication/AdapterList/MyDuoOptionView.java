...
public class MyDuoOptionView extends RelativeLayout {
    private com.e.prismetrictestapplication.AdapterList.MyDuoOptionView.OptionViewHolder mOptionViewHolder;

    private static final float ALPHA_CHECKED = 1f;
    private static final float ALPHA_UNCHECKED = 1f;

    private boolean mIsSideSelectorEnabled = false;
    private boolean mIsSelectorEnabled = false;

    public MyDuoOptionView(Context context) {
        this(context, null);
    }

    public MyDuoOptionView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyDuoOptionView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initialize();
    }

    private void initialize() {
        ViewGroup rootView = (ViewGroup) inflate(getContext(), R.layout.my_duo_view_option, this);

        mOptionViewHolder = new com.e.prismetrictestapplication.AdapterList.MyDuoOptionView.OptionViewHolder(rootView);
    }
...
//        setSideSelectorEnabled(true);
    ...
    private class OptionViewHolder {
        ...
        OptionViewHolder(ViewGroup rootView) {
            ...
            mImageArrow = (ImageView) rootView.findViewById(R.id.duo_view_option_right_arrow);
